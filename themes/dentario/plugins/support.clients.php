<?php
/**
 * Dentario Framework: Clients support
 *
 * @package	dentario
 * @since	dentario 1.0
 */
...
// Theme init
if (!function_exists('dentario_clients_theme_setup')) {
	add_action( 'dentario_action_before_init_theme', 'dentario_clients_theme_setup', 1 );
	function dentario_clients_theme_setup() {

		// Detect current page type, taxonomy and title (for custom post_types use priority < 10 to fire it handles early, than for standard post types)
		add_filter('dentario_filter_get_blog_type',			'dentario_clients_get_blog_type', 9, 2);
		add_filter('dentario_filter_get_blog_title',		'dentario_clients_get_blog_title', 9, 2);
		add_filter('dentario_filter_get_current_taxonomy',	'dentario_clients_get_current_taxonomy', 9, 2);
		add_filter('dentario_filter_is_taxonomy',			'dentario_clients_is_taxonomy', 9, 2);
		add_filter('dentario_filter_get_stream_page_title',	'dentario_clients_get_stream_page_title', 9, 2);
		add_filter('dentario_filter_get_stream_page_link',	'dentario_clients_get_stream_page_link', 9, 2);
		add_filter('dentario_filter_get_stream_page_id',	'dentario_clients_get_stream_page_id', 9, 2);
		add_filter('dentario_filter_query_add_filters',		'dentario_clients_query_add_filters', 9, 2);
		add_filter('dentario_filter_detect_inheritance_key','dentario_clients_detect_inheritance_key', 9, 1);

		// Extra column for clients lists
		if (dentario_get_theme_option('show_overriden_posts')=='yes') {
			add_filter('manage_edit-clients_columns',			'dentario_post_add_options_column', 9);
			add_filter('manage_clients_posts_custom_column',	'dentario_post_fill_options_column', 9, 2);
		}

		// Registar shortcodes [trx_clients] and [trx_clients_item] in the shortcodes list
		add_action('dentario_action_shortcodes_list',		'dentario_clients_reg_shortcodes');
		if (function_exists('dentario_exists_visual_composer') && dentario_exists_visual_composer())
			add_action('dentario_action_shortcodes_list_vc','dentario_clients_reg_shortcodes_vc');
		
		// Add supported data types
		dentario_theme_support_pt('clients');
		dentario_theme_support_tx('clients_group');
	}
}

if ( !function_exists( 'dentario_clients_settings_theme_setup2' ) ) {
	add_action( 'dentario_action_before_init_theme', 'dentario_clients_settings_theme_setup2', 3 );
...
if (!function_exists('dentario_clients_after_theme_setup')) {
	add_action( 'dentario_action_after_init_theme', 'dentario_clients_after_theme_setup' );
	function dentario_clients_after_theme_setup() {
		// Update fields in the meta box
		if (dentario_storage_get_array('post_meta_box', 'page')=='clients') {
			// Meta box fields
			dentario_storage_set_array('post_meta_box', 'title', esc_html__('Client Options', 'dentario'));
			dentario_storage_set_array('post_meta_box', 'fields', array(
				"mb_partition_clients" => array(
					"title" => esc_html__('Clients', 'dentario'),
					"override" => "page,post,custom",
					"divider" => false,
					"icon" => "iconadmin-users",
					"type" => "partition"),
				"mb_info_clients_1" => array(
					"title" => esc_html__('Client details', 'dentario'),
					"override" => "page,post,custom",
					"divider" => false,
					"desc" => wp_kses_data( __('In this section you can put details for this client', 'dentario') ),
					"class" => "client_meta",
					"type" => "info"),
				"client_name" => array(
					"title" => esc_html__('Contact name',  'dentario'),
					"desc" => wp_kses_data( __("Name of the contacts manager", 'dentario') ),
					"override" => "page,post,custom",
					"class" => "client_name",
					"std" => '',
					"type" => "text"),
				"client_position" => array(
					...
					"desc" => wp_kses_data( __("Position of the contacts manager", 'dentario') ),
					"override" => "page,post,custom",
					"class" => "client_position",
					"std" => '',
					"type" => "text"),
				"client_show_link" => array(
					"title" => esc_html__('Show link',  'dentario'),
					"desc" => wp_kses_data( __("Show link to client page", 'dentario') ),
					"override" => "page,post,custom",
					"class" => "client_show_link",
					"std" => "no",
					"options" => dentario_get_list_yesno(),
					"type" => "switch"),
				"client_link" => array(
					"title" => esc_html__('Link',  'dentario'),
					"desc" => wp_kses_data( __("URL of the client's site. If empty - use link to this page", 'dentario') ),
					"override" => "page,post,custom",
					"class" => "client_link",
					"std" => '',
					"type" => "text")
...
if ( !function_exists( 'dentario_is_clients_page' ) ) {
	function dentario_is_clients_page() {
		$is = in_array(dentario_storage_get('page_template'), array('blog-clients', 'single-client'));
		if (!$is) {
			if (!dentario_storage_empty('pre_query'))
				$is = dentario_storage_call_obj_method('pre_query', 'get', 'post_type')=='clients'
						|| dentario_storage_call_obj_method('pre_query', 'is_tax', 'clients_group') 
						|| (dentario_storage_call_obj_method('pre_query', 'is_page') 
							&& ($id=dentario_get_template_page_id('blog-clients')) > 0 
							&& $id==dentario_storage_get_obj_property('pre_query', 'queried_object_id', 0)
							);
			else
				$is = get_query_var('post_type')=='clients' 
						|| is_tax('clients_group') 
						|| (is_page() && ($id=dentario_get_template_page_id('blog-clients')) > 0 && $id==get_the_ID());
		}
		return $is;
	}
}
...
if ( !function_exists( 'dentario_clients_detect_inheritance_key' ) ) {
	//add_filter('dentario_filter_detect_inheritance_key',	'dentario_clients_detect_inheritance_key', 9, 1);
	function dentario_clients_detect_inheritance_key($key) {
		if (!empty($key)) return $key;
		return dentario_is_clients_page() ? 'clients' : '';
	}
}

// Filter to detect current page slug
if ( !function_exists( 'dentario_clients_get_blog_type' ) ) {
	//add_filter('dentario_filter_get_blog_type',	'dentario_clients_get_blog_type', 9, 2);
	function dentario_clients_get_blog_type($page, $query=null) {
...
if ( !function_exists( 'dentario_clients_get_blog_title' ) ) {
	//add_filter('dentario_filter_get_blog_title',	'dentario_clients_get_blog_title', 9, 2);
	function dentario_clients_get_blog_title($title, $page) {
		if (!empty($title)) return $title;
		if ( dentario_strpos($page, 'clients')!==false ) {
			if ( $page == 'clients_category' ) {
				...
				$title = esc_html__('All clients', 'dentario');
			}
		}
		return $title;
	}
...
if ( !function_exists( 'dentario_clients_get_stream_page_title' ) ) {
	//add_filter('dentario_filter_get_stream_page_title',	'dentario_clients_get_stream_page_title', 9, 2);
	function dentario_clients_get_stream_page_title($title, $page) {
		if (!empty($title)) return $title;
		if (dentario_strpos($page, 'clients')!==false) {
			if (($page_id = dentario_clients_get_stream_page_id(0, $page=='clients' ? 'blog-clients' : $page)) > 0)
...
if ( !function_exists( 'dentario_clients_get_stream_page_id' ) ) {
	//add_filter('dentario_filter_get_stream_page_id',	'dentario_clients_get_stream_page_id', 9, 2);
	function dentario_clients_get_stream_page_id($id, $page) {
		if (!empty($id)) return $id;
		if (dentario_strpos($page, 'clients')!==false) $id = dentario_get_template_page_id('blog-clients');
		return $id;
	}
}

// Filter to detect stream page URL
if ( !function_exists( 'dentario_clients_get_stream_page_link' ) ) {
	//add_filter('dentario_filter_get_stream_page_link',	'dentario_clients_get_stream_page_link', 9, 2);
	function dentario_clients_get_stream_page_link($url, $page) {
		if (!empty($url)) return $url;
		if (dentario_strpos($page, 'clients')!==false) {
			$id = dentario_get_template_page_id('blog-clients');
...
if ( !function_exists( 'dentario_clients_get_current_taxonomy' ) ) {
	//add_filter('dentario_filter_get_current_taxonomy',	'dentario_clients_get_current_taxonomy', 9, 2);
	function dentario_clients_get_current_taxonomy($tax, $page) {
...
if ( !function_exists( 'dentario_clients_is_taxonomy' ) ) {
	//add_filter('dentario_filter_is_taxonomy',	'dentario_clients_is_taxonomy', 9, 2);
	function dentario_clients_is_taxonomy($tax, $query=null) {
		if (!empty($tax))
			return $tax;
		else 
...
if ( !function_exists( 'dentario_clients_query_add_filters' ) ) {
	//add_filter('dentario_filter_query_add_filters',	'dentario_clients_query_add_filters', 9, 2);
	function dentario_clients_query_add_filters($args, $filter) {
		if ($filter == 'clients') {
...
[/trx_clients]
*/
if ( !function_exists( 'dentario_sc_clients' ) ) {
	function dentario_sc_clients($atts, $content=null){	
		...
		dentario_storage_set('sc_clients_data', array(
			'id'=>$id,
            'style'=>$style,
            'counter'=>0,
            'columns'=>$columns,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '') 
					...
					. '>'
					. (!empty($subtitle) ? '<h6 class="sc_clients_subtitle sc_item_subtitle">' . trim(dentario_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_clients_title sc_item_title">' . trim(dentario_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_clients_descr sc_item_descr">' . trim(dentario_strmacros($description)) . '</div>' : '')
		...
		if (dentario_param_is_on($custom) && $content) {
			...
			$args = dentario_query_add_posts_and_cats($args, $ids, 'clients', $cat, 'clients_group');
			...
			while ( $query->have_posts() ) { 
				...
				$post_meta = get_post_meta($post_data['post_id'], 'dentario_post_options', true);
				...
				$args['client_link'] = dentario_param_is_on('client_show_link')
		...
		$output .= (!empty($link) ? '<div class="sc_clients_button sc_item_button">'.dentario_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
				. '</div><!-- /.sc_clients -->'
			. '</div><!-- /.sc_clients_wrap -->';
		...
		return apply_filters('dentario_shortcode_output', $output, 'trx_clients', $atts, $content);
	}
	dentario_require_shortcode('trx_clients', 'dentario_sc_clients');
}


if ( !function_exists( 'dentario_sc_clients_item' ) ) {
	function dentario_sc_clients_item($atts, $content=null) {
		if (dentario_in_shortcode_blogger()) return '';
		extract(dentario_html_decode(shortcode_atts( array(
			// Individual params
			"name" => "",
		...
		dentario_storage_inc_array('sc_clients_data', 'counter');
	
		$id = $id ? $id : (dentario_storage_get_array('sc_clients_data', 'id') ? dentario_storage_get_array('sc_clients_data', 'id') . '_' . dentario_storage_get_array('sc_clients_data', 'counter') : '');
	
		$descr = trim(chop(do_shortcode($content)));
	
		$thumb_sizes = dentario_get_thumb_sizes(array('layout' => dentario_storage_get_array('sc_clients_data', 'style')));
		...
		$args = array(
			'layout' => dentario_storage_get_array('sc_clients_data', 'style'),
			'number' => dentario_storage_get_array('sc_clients_data', 'counter'),
			'columns_count' => dentario_storage_get_array('sc_clients_data', 'columns'),
			'slider' => dentario_storage_get_array('sc_clients_data', 'slider'),
			'show' => false,
			'descr'  => 0,
			'tag_id' => $id,
			'tag_class' => $class,
			...
			'tag_css' => $css,
			'tag_css_wh' => dentario_storage_get_array('sc_clients_data', 'css_wh'),
			'client_position' => $position,
			'client_link' => $link,
			'client_image' => $image
		);
		$output = dentario_show_post_layout($args, $post_data);
		return apply_filters('dentario_shortcode_output', $output, 'trx_clients_item', $atts, $content);
	}
	dentario_require_shortcode('trx_clients_item', 'dentario_sc_clients_item');
}
// ---------------------------------- [/trx_clients] ---------------------------------------
...
if (!function_exists('dentario_clients_reg_shortcodes')) {
	//add_filter('dentario_action_shortcodes_list',	'dentario_clients_reg_shortcodes');
	function dentario_clients_reg_shortcodes() {
		if (dentario_storage_isset('shortcodes')) {
			...
			$clients_groups = dentario_get_list_terms(false, 'clients_group');
			$clients_styles = dentario_get_list_templates('clients');
			$controls 		= dentario_get_list_slider_controls();

			dentario_sc_map_after('trx_chat', array(

				// Clients
				"trx_clients" => array(
					"title" => esc_html__("Clients", 'dentario'),
					"desc" => wp_kses_data( __("Insert clients list in your page (post)", 'dentario') ),
					"decorate" => true,
					"container" => false,
					"params" => array(
						"title" => array(
						...
						"style" => array(
							"title" => esc_html__("Clients style", 'dentario'),
							"desc" => wp_kses_data( __("Select style to display clients list", 'dentario') ),
							"value" => "clients-1",
							"type" => "select",
						...
						"columns" => array(
							...
							"desc" => wp_kses_data( __("How many columns use to show clients", 'dentario') ),
							"value" => 4,
							"min" => 2,
							"max" => 6,
							"step" => 1,
						...
						"slider" => array(
							...
							"desc" => wp_kses_data( __("Use slider to show clients", 'dentario') ),
							"value" => "no",
							"type" => "switch",
						...
						"cat" => array(
							...
							"style" => "list",
							"multiple" => true,
							"options" => dentario_array_merge(array(0 => esc_html__('- Select category -', 'dentario')), $clients_groups)
						),
						"count" => array(
					...
					"children" => array(
						...
						"title" => esc_html__("Client", 'dentario'),
						"desc" => wp_kses_data( __("Single client (custom parameters)", 'dentario') ),
						"container" => true,
						"params" => array(
							"name" => array(
								"title" => esc_html__("Name", 'dentario'),
								"desc" => wp_kses_data( __("Client's name", 'dentario') ),
								"divider" => true,
								"value" => "",
								"type" => "text"
							),
							"position" => array(
								"title" => esc_html__("Position", 'dentario'),
								"desc" => wp_kses_data( __("Client's position", 'dentario') ),
								"value" => "",
								"type" => "text"
							),
							"link" => array(
								"title" => esc_html__("Link", 'dentario'),
								"desc" => wp_kses_data( __("Link on client's personal page", 'dentario') ),
								"divider" => true,
								"value" => "",
								"type" => "text"
							),
							"image" => array(
								...
								"desc" => wp_kses_data( __("Client's image", 'dentario') ),
								"value" => "",
								"readonly" => false,
								"type" => "media"
							),
							"_content_" => array(
								"title" => esc_html__("Description", 'dentario'),
								"desc" => wp_kses_data( __("Client's short description", 'dentario') ),
								"divider" => true,
								"rows" => 4,
								"value" => "",
								"type" => "textarea"
...
if (!function_exists('dentario_clients_reg_shortcodes_vc')) {
	//add_filter('dentario_action_shortcodes_list_vc',	'dentario_clients_reg_shortcodes_vc');
	function dentario_clients_reg_shortcodes_vc() {
		...
		vc_map( array(
				"base" => "trx_clients",
				"name" => esc_html__("Clients", 'dentario'),
				"description" => wp_kses_data( __("Insert clients list", 'dentario') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("Clients style", 'dentario'),
						"description" => wp_kses_data( __("Select style to display clients list", 'dentario') ),
						"class" => "",
						"admin_label" => true,
					...
					array(
						...
						"description" => wp_kses_data( __("Allow get clients from inner shortcodes (custom) or get it from specified group (cat)", 'dentario') ),
						"class" => "",
					...
					array(
						...
						"description" => wp_kses_data( __("Select category to show clients. If empty - select clients from any category (group) or from IDs list", 'dentario') ),
						...
						"class" => "",
						"value" => array_flip(dentario_array_merge(array(0 => esc_html__('- Select category -', 'dentario')), $clients_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
						"heading" => esc_html__("Columns", 'dentario'),
						"description" => wp_kses_data( __("How many columns use to show clients", 'dentario') ),
						"group" => esc_html__('Query', 'dentario'),
						"admin_label" => true,
						"class" => "",
						"value" => "4",
					...
					array(
						"param_name" => "ids",
						"heading" => esc_html__("client's IDs list", 'dentario'),
						"description" => wp_kses_data( __("Comma separated list of client's ID. If set - parameters above (category, count, order, etc.)  are ignored!", 'dentario') ),
						"group" => esc_html__('Query', 'dentario'),
						'dependency' => array(
							'element' => 'custom',
							'is_empty' => true
		...
		vc_map( array(
				...
				"name" => esc_html__("Client", 'dentario'),
				"description" => wp_kses_data( __("Client - all data pull out from it account on your site", 'dentario') ),
				"show_settings_on_create" => true,
				...
				"params" => array(
					array(
						...
						"description" => wp_kses_data( __("Client's name", 'dentario') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"description" => wp_kses_data( __("Client's position", 'dentario') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"description" => wp_kses_data( __("Link on client's personal page", 'dentario') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "image",
						"heading" => esc_html__("Client's image", 'dentario'),
						"description" => wp_kses_data( __("Clients's image", 'dentario') ),
						"class" => "",
						"value" => "",
						"type" => "attach_image"
					),
		...
		class WPBakeryShortCode_Trx_Clients extends DENTARIO_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Clients_Item extends DENTARIO_VC_ShortCodeCollection {}
