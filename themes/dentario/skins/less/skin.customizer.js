...
function dentario_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = DENTARIO_STORAGE['co_add_params']!=undefined ? DENTARIO_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#dentario-customizer-styles-'+option).length > 0 ? jQuery('#dentario-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="dentario-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
