...
if ( !function_exists( 'dentario_template_attachment_theme_setup' ) ) {
	add_action( 'dentario_action_before_init_theme', 'dentario_template_attachment_theme_setup', 1 );
	function dentario_template_attachment_theme_setup() {
		dentario_add_template(array(
			'layout' => 'attachment',
			'mode'   => 'internal',
			'title'  => esc_html__('Attachment page layout', 'dentario'),
...
// Template output
if ( !function_exists( 'dentario_template_attachment_output' ) ) {
	function dentario_template_attachment_output($post_options, $post_data) {
		...
		<article <?php post_class('post_item post_item_attachment template_attachment'); ?>>
			...
			<div class="post_featured">
				<div class="post_thumb post_nav" data-image="<?php echo esc_url($post_data['post_attachment']); ?>" data-title="<?php echo esc_attr($post_data['post_title']); ?>">
					...
					if ( isset( $attachments[ $k-1 ] ) ) {
						...
						$desc = dentario_strshort(!empty($attachments[ $k-1 ]->post_excerpt) ? $attachments[ $k-1 ]->post_excerpt : $attachments[ $k-1 ]->post_title, 30);
						?>
						<a class="post_nav_item post_nav_prev" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Previous item', 'dentario'); ?></span>
					...
					if ( isset( $attachments[ $k+1 ] ) ) {
						...
						<a class="post_nav_item post_nav_next" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Next item', 'dentario'); ?></span>
								<span class="post_nav_info_description"><?php echo trim($desc); ?></span>
							</span>
						</a>
						<?php
			...
			<div class="post_content">
				...
				if ($post_data['post_protected']) { 
					...
					echo !empty($post_data['post_content']) ? trim($post_data['post_content']) : esc_html__('No image description ...', 'dentario'); 
					wp_link_pages( array( 
						'before' => '<div class="nav_pages_parts"><span class="pages">' . esc_html__( 'Pages:', 'dentario' ) . '</span>', 
						'after' => '</div>',
