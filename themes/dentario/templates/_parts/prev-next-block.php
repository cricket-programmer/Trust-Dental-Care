<?php
// Get template args
extract(dentario_template_get_args('prev-next-block'));
...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = dentario_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo 'single-portfolio-fullscreen'==$page_style ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''; ?>>
			...
			$args = dentario_query_add_posts_and_cats($args, '', $post_data['post_type'], 
			...
			if ( $prev ) {
				...
				$desc = dentario_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = dentario_strshort($next->post_title, 30);
				?>
