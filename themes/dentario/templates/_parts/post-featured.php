<?php
// Get template args
extract(dentario_template_get_args('post-featured'));

if (!empty($post_data['post_video'])) {
	echo trim(dentario_get_video_frame($post_data['post_video'], $post_data['post_video_image'] ? $post_data['post_video_image'] : $post_data['post_thumb']));
} else if (!empty($post_data['post_audio'])) {
	if (dentario_get_custom_option('substitute_audio')=='no' || !dentario_in_shortcode_blogger(true))
		echo trim(dentario_get_audio_frame($post_data['post_audio'], $post_data['post_audio_image'] ? $post_data['post_audio_image'] : $post_data['post_thumb_url']));
	else
		echo trim($post_data['post_audio']);
} else if (!empty($post_data['post_thumb']) && ($post_data['post_format']!='gallery' || !$post_data['post_gallery'] || dentario_get_custom_option('gallery_instead_image')=='no')) {
	?>
