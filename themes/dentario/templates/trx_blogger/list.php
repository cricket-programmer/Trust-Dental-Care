...
if ( !function_exists( 'dentario_template_list_theme_setup' ) ) {
	add_action( 'dentario_action_before_init_theme', 'dentario_template_list_theme_setup', 1 );
	function dentario_template_list_theme_setup() {
		dentario_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'dentario')
			));
	}
}

// Template output
if ( !function_exists( 'dentario_template_list_output' ) ) {
	function dentario_template_list_output($post_options, $post_data) {
