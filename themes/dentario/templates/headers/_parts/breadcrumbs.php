<?php
$show_title = dentario_get_custom_option('show_page_title')=='yes';
$show_navi = $show_title && is_single() && dentario_is_woocommerce_page();
$show_breadcrumbs = dentario_get_custom_option('show_breadcrumbs')=='yes';
			...
			<div class="content_wrap">
				...
				if ($show_title) {
					if ($show_navi) {
						...
						?><h1 class="page_title"><?php echo strip_tags(dentario_get_blog_title()); ?></h1><?php
					}
				}
				if ($show_breadcrumbs) {
					?><div class="breadcrumbs"><?php if (!is_404()) dentario_show_breadcrumbs(); ?></div><?php
				}
				?>
			</div>
		</div>
