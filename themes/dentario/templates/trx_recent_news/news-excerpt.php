...
if ( !function_exists( 'dentario_template_news_excerpt_theme_setup' ) ) {
	add_action( 'dentario_action_before_init_theme', 'dentario_template_news_excerpt_theme_setup', 1 );
	function dentario_template_news_excerpt_theme_setup() {
		dentario_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'dentario'),
...
// Template output
if ( !function_exists( 'dentario_template_news_excerpt_output' ) ) {
	function dentario_template_news_excerpt_output($post_options, $post_data) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(dentario_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: dentario_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}
