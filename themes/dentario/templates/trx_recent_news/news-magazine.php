...
if ( !function_exists( 'dentario_template_news_magazine_theme_setup' ) ) {
	add_action( 'dentario_action_before_init_theme', 'dentario_template_news_magazine_theme_setup', 1 );
	function dentario_template_news_magazine_theme_setup() {
		dentario_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'dentario'),
...
// Template output
if ( !function_exists( 'dentario_template_news_magazine_output' ) ) {
	function dentario_template_news_magazine_output($post_options, $post_data) {
		...
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
			...
			if ($number <= $featured) {
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(dentario_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
				...
				<div class="post_footer entry-footer">
					...
					<?php
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						$post_options['counters'] = 'views,comments,edit,captions';		//dentario_get_theme_option('blog_counters');
