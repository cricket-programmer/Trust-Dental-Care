...
if ( !function_exists( 'dentario_template_services_4_theme_setup' ) ) {
	add_action( 'dentario_action_before_init_theme', 'dentario_template_services_4_theme_setup', 1 );
	function dentario_template_services_4_theme_setup() {
		dentario_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'dentario')
		));
	}
}

// Template output
if ( !function_exists( 'dentario_template_services_4_output' ) ) {
	function dentario_template_services_4_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				echo trim(dentario_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
