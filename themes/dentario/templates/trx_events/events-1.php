...
if ( !function_exists( 'dentario_template_events_1_theme_setup' ) ) {
	add_action( 'dentario_action_before_init_theme', 'dentario_template_events_1_theme_setup', 1 );
	function dentario_template_events_1_theme_setup() {
		dentario_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'dentario'),
...
// Template output
if ( !function_exists( 'dentario_template_events_1_output' ) ) {
	function dentario_template_events_1_output($post_options, $post_data) {
		$show_title = true;
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(dentario_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																										? $post_options['descr'] 
																										: dentario_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}
