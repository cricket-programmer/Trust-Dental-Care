...
if ( !function_exists( 'dentario_template_events_2_theme_setup' ) ) {
	add_action( 'dentario_action_before_init_theme', 'dentario_template_events_2_theme_setup', 1 );
	function dentario_template_events_2_theme_setup() {
		dentario_add_template(array(
			'layout' => 'events-2',
			'template' => 'events-2',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 2/', 'dentario')
		));
	}
}

// Template output
if ( !function_exists( 'dentario_template_events_2_output' ) ) {
	function dentario_template_events_2_output($post_options, $post_data) {
		$show_title = true;
		...
		<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
			...
			?><span class="sc_events_item_time"><?php
				echo (trim($start_date[1]) ? $start_date[1] : esc_html__('Whole day', 'dentario'))
