...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		dentario_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function dentario_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: DENTARIO_STORAGE['ajax_nonce'],
		action: 'dentario_importer_start_import',
	...
	jQuery.post(DENTARIO_STORAGE['ajax_url'], data, function(response) {
		...
		try {
			...
			rez = { error: DENTARIO_STORAGE['ajax_error']+':<br>'+response };
		...
		if (rez.error === '') {
			...
			// Do next action
			if (idx < steps.length) {
				dentario_importer_do_action(steps, idx);
			} else {
