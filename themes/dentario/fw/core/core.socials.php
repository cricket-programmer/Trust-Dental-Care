<?php
/**
 * Dentario Framework: social networks
 *
 * @package	dentario
 * @since	dentario 1.0
 */
...
// Theme init
if (!function_exists('dentario_socials_theme_setup')) {
	add_action( 'dentario_action_before_init_theme', 'dentario_socials_theme_setup' );
	function dentario_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'dentario_facebook_og_tags', 5 );
		}
...
dentario_storage_set('share_links', array(
...
//	'facebook' =>		'http://www.facebook.com/sharer.php?s=100&p[url]={link}&p[title]={title}&p[summary]={descr}&p[images][0]={image}',
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=dentario',
...
if (!function_exists('dentario_add_share_link')) {
	function dentario_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		dentario_storage_set('share_links', array_merge( dentario_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('dentario_show_share_links')) {
	function dentario_show_share_links($args) {
		if ( dentario_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'style' => dentario_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons',	// style for show icons: icons|images|bg
			'type' => 'block',					// share block type: list|block|drop
			'popup' => true,					// open share url in new window or in popup window
			'counters' => dentario_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => dentario_get_custom_option('show_share'),				// share block direction
			'caption' => dentario_get_custom_option('share_caption'),				// share block caption
			'share' => dentario_get_theme_option('share_buttons'),					// list of allowed socials
		...
		$output = '<div class="sc_socials sc_socials_size_'.esc_attr($args['size']).' sc_socials_share' . ($args['type']=='drop' ? ' sc_socials_drop' : ' sc_socials_dir_' . esc_attr($args['direction'])) . '">'
			. ($args['caption']!='' ? '<div class="share_caption">'.($args['caption']) : '');

		if (is_array($args['share']) && count($args['share']) > 0) {
			$output .= '<div class="sc_social_items_block">';
			foreach ($args['share'] as $soc) {
				$icon = $args['style']=='icons' || dentario_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : dentario_get_socials_url(basename($soc['icon']));
				if ($args['style'] == 'icons') {
				...
				$url = $soc['url'];
				if (empty($url)) $url = dentario_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(dentario_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div></div></div>';
...
if (!function_exists('dentario_prepare_socials')) {
	function dentario_prepare_socials($list, $style='') {
		if (empty($style)) $style = dentario_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || dentario_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : dentario_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('dentario_get_twitter_data')) {
	function dentario_get_twitter_data($cfg) {
...
if (!function_exists('dentario_prepare_twitter_text')) {
	function dentario_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('dentario_get_twitter_followers')) {
	function dentario_get_twitter_followers($cfg) {
		$data = dentario_get_twitter_data($cfg); 
...
if (!function_exists('dentario_get_facebook_likes')) {
	function dentario_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(dentario_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (dentario_strpos($headers[0], '200')) {
			$json = dentario_fgc($url);
...
if (!function_exists('dentario_facebook_og_tags')) {
	function dentario_facebook_og_tags() {
		global $post;
		if ( !is_singular() || dentario_storage_get('blog_streampage')) return;
...
if (!function_exists('dentario_get_feedburner_counter')) {
	function dentario_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(dentario_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (dentario_strpos($headers[0], '200')) {
			$xml = dentario_fgc($url);
