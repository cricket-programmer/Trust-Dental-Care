<?php
/**
 * Dentario Framework: WordPress utilities
 *
 * @package	dentario
 * @since	dentario 1.0
 */
...
// Theme init
if (!function_exists('dentario_wp_theme_setup')) {
	add_action( 'dentario_action_before_init_theme', 'dentario_wp_theme_setup' );
	function dentario_wp_theme_setup() {

		// Pre query: Add posts per page value into wp query
		add_action( 'pre_get_posts', 						'dentario_query_add_posts_per_page' );

		// Pre query: Add filter to main query
		add_filter('posts_where', 							'dentario_query_posts_where', 10, 2);  

		// Get theme calendar (instead standard WP calendar) to support Events
		add_filter( 'get_calendar',							'dentario_get_calendar' );

		// Filter categories list - exclude unwanted cats from widget output
		add_action('widget_categories_args',				'dentario_query_exclude_categories');
		add_action('widget_categories_dropdown_args',		'dentario_query_exclude_categories');
		add_action('getarchives_where',						'dentario_query_exclude_archives_where', 10, 2);
		add_action('widget_posts_args',						'dentario_query_exclude_posts' );

		// AJAX: Get next page on blog streampage
		add_action('wp_ajax_view_more_posts', 				'dentario_callback_view_more_posts');
		add_action('wp_ajax_nopriv_view_more_posts',		'dentario_callback_view_more_posts');

		// AJAX: Incremental search
		add_action('wp_ajax_ajax_search',					'dentario_callback_ajax_search');
		add_action('wp_ajax_nopriv_ajax_search',			'dentario_callback_ajax_search');

		// AJAX: Change month in the calendar widget
		add_action('wp_ajax_calendar_change_month',			'dentario_callback_calendar_change_month');
		add_action('wp_ajax_nopriv_calendar_change_month',	'dentario_callback_calendar_change_month');

		// AJAX: Set post likes/views count
		add_action('wp_ajax_post_counter', 					'dentario_callback_post_counter');
		add_action('wp_ajax_nopriv_post_counter', 			'dentario_callback_post_counter');

		// AJAX: New user registration
		add_action('wp_ajax_registration_user',				'dentario_callback_registration_user');
		add_action('wp_ajax_nopriv_registration_user',		'dentario_callback_registration_user');

		// AJAX: User login
		add_action('wp_ajax_login_user',					'dentario_callback_login_user');
		add_action('wp_ajax_nopriv_login_user',				'dentario_callback_login_user');

		// Allow user login by email from WP login dialog
		add_filter('authenticate',							'dentario_allow_email_login', 20, 3);

		// Add categories (taxonomies) filter for custom posts types
		add_action( 'restrict_manage_posts', 				'dentario_show_taxonomies_filters' );

		// Clear taxonomies cache when save or delete post or category
		add_action( 'save_post', 							'dentario_clear_cache_all', 10, 2 );
		add_action( 'delete_post', 							'dentario_clear_cache_all', 10, 2 );
		add_action( 'edit_category', 						'dentario_clear_cache_categories',   10, 1 );
		add_action( 'create_category', 						'dentario_clear_cache_categories', 10, 1 );
		add_action( 'delete_category', 						'dentario_clear_cache_categories', 10, 1 );

		// Clear menu cache
		add_action('wp_update_nav_menu',					'dentario_clear_cache_menu', 10, 2);

		// Clear WP categories cache 
		add_action('wp_ajax_clear_cache',					'dentario_callback_clear_cache');
		add_action('wp_ajax_nopriv_clear_cache', 			'dentario_callback_clear_cache');
	}
}
...
if (!function_exists('dentario_get_blog_type')) {
	function dentario_get_blog_type($query=null) {
		global $wp_query;
		static $page='';
		if (!empty($page)) return $page;
		if ( $query===null ) $query = $wp_query;
		$page = apply_filters('dentario_filter_get_blog_type', $page, $query);
		return $page;
	}
}

// Return blog title
if (!function_exists('dentario_get_blog_title')) {
	function dentario_get_blog_title() {
		global $wp_query;
		static $title='';
		if (!empty($title)) return $title;
		$page  = dentario_get_blog_type();
		$title = apply_filters('dentario_filter_get_blog_title', $title, $page);
		return $title;
	}
}

// Show breadcrumbs path
if (!function_exists('dentario_show_breadcrumbs')) {
	function dentario_show_breadcrumbs($args=array()) {
		global $wp_query, $post;
		
		dentario_storage_set('in_breadcrumbs', true);
		
		$args = array_merge( array(
			'home' => esc_html__('Home', 'dentario'),		// Home page title (if empty - not showed)
			...
			'max_levels' => dentario_get_theme_option('breadcrumbs_max_level'),		// Max categories in the path (0 - unlimited)
			'echo' => true							// If true - show on page, else - only return value
			), is_array($args) ? $args : array( 'home' => $args )
		);

		$type = dentario_get_blog_type();
		$title = dentario_strshort(dentario_get_blog_title(), $args['truncate_title'], $args['truncate_add']);
			...
			while ($pageParentID > 0) {
				...
				if ($level > $args['max_levels'])
					...
					$rez_parent = '<a class="breadcrumbs_item cat_post" href="' . get_permalink($pageParent->ID) . '">' 
									. trim(dentario_strshort($pageParent->post_title, $args['truncate_title'], $args['truncate_add']))
									. '</a>' 
		...
		$taxonomy = apply_filters('dentario_filter_get_current_taxonomy', '', $type);
		$ex_cats = $taxonomy == 'category' ? explode(',', dentario_get_theme_option('exclude_cats')) : array();
			...
			if ($depth++ == 0) {
				if (is_single() || is_attachment()) {
					if ($args['show_all_filters']) {
						...
						if (($tpl_id = dentario_get_template_page_id('only-'.($post_format))) > 0) {
							$level++;
							if ($level > $args['max_levels'])
								$rez_level = '...';
							else
								$rez_all .= (!empty($rez_all) ? $args['delimiter'] : '') 
										. '<a class="breadcrumbs_item all" href="' . get_permalink($tpl_id) . '">' 
											. sprintf(esc_html__('All %s', 'dentario'), dentario_get_post_format_name($post_format, false)) 
										. '</a>';
						}
					}
					$cats = dentario_get_terms_by_post_id( array(
						'post_id'  => $post_id, 
						'taxonomy' => $taxonomy,
						'exclude'  => $ex_cats
						)
					...
					if ($cat) {
						...
						if ($level > $args['max_levels'])
							...
							$rez_parent = '<a class="breadcrumbs_item cat_post" href="'.esc_url($cat->link).'">' 
											. trim(dentario_strshort($cat->name, $args['truncate_title'], $args['truncate_add']))
											. '</a>' 
											. (!empty($rez_parent) ? $args['delimiter'] : '') 
											. ($rez_parent);
					}
				...
				} else if ( ($tax = dentario_is_taxonomy()) != '' ) {
		...
		if ($args['show_all_posts']) {
			$all_title = dentario_get_stream_page_title($type);
			$all_parts = explode(':', $all_title);
			if (count($all_parts) > 1)
				$all_title = trim($all_parts[1]);
			if ( $all_title && dentario_strtolower($title) != dentario_strtolower($all_title)) {
				$all_link = dentario_get_stream_page_link($type);
				if (!empty($all_link))
					$rez_all = '<a class="breadcrumbs_item all" href="' . esc_url($all_link) . '">' . ($all_title) . '</a>' . (!empty($rez_all) ? $args['delimiter'] : '') . ($rez_all);
			}
		}

		$rez_period = apply_filters('dentario_filter_get_period_links', '', $type, $args['delimiter']);
		...
		dentario_storage_set('in_breadcrumbs', false);
...
if (!function_exists('dentario_show_pagination')) {
	function dentario_show_pagination($args=array()) {
		$args = array_merge(array(
			...
			'show_pages' => 5,
			'pages_in_group' => 10,
			'pages_text' => '', 		//esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'dentario'),
			'current_text' => "%PAGE_NUMBER%",
			'page_text' => "%PAGE_NUMBER%",
			'first_text'=> esc_html__('&laquo; First', 'dentario'),
			'last_text' => esc_html__("Last &raquo;", 'dentario'),
			'prev_text' => esc_html__("&laquo; Prev", 'dentario'),
			'next_text' => esc_html__("Next &raquo;", 'dentario'),
			'dot_text' => "&hellip;",
			'before' => '',
			'after' => '',
			),  is_array($args) ? $args 
		...
		if (!is_single()) {
			dentario_show_pagination_blog($args);
		} else {
			dentario_show_pagination_single($args);
		}
	}
}

// Single page navigation
if (!function_exists('dentario_show_pagination_single')) {
	function dentario_show_pagination_single( $args ) {
		global $wp_query, $post;
		...
		if ( is_single() ) {
			?>
			<h3 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'dentario' ); ?></h3>
			<?php
...
// Blog pagination
if (!function_exists('dentario_show_pagination_blog')) {
	function dentario_show_pagination_blog($opt) {
		global $wp_query;
			...
			if ($max_page > 1) {
				...
				if ($opt['style'] == 'pages') {
					...
					if (empty($opt['pages_text'])) 
						$opt['pages_text'] = esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'dentario');
...
if ( !function_exists( 'dentario_callback_view_more_posts' ) ) {
	function dentario_callback_view_more_posts() {
		...
		$page = $_REQUEST['page'];
		$args = dentario_unserialize(stripslashes($_REQUEST['data']));
		$vars = dentario_unserialize(stripslashes($_REQUEST['vars']));
		...
		if ($page > 0 && is_array($args) && is_array($vars)) {
			...
			while ( have_posts() ) { the_post(); 
				...
				$post_args = array(
					...
					// Get post data
					'content' => dentario_get_template_property($vars['blog_style'], 'need_content'),
					'terms_list' => !dentario_param_is_off($vars['filters']) || dentario_get_template_property($vars['blog_style'], 'need_terms'),
					'strip_teaser' => false,
				...
				$post_data = dentario_get_post_data($post_args);
				$response['data'] .= dentario_show_post_layout($post_args, $post_data);
			...
			$response['error'] = esc_html__('Wrong query arguments', 'dentario');
		}
...
// Incremental search
if ( !function_exists( 'dentario_callback_ajax_search' ) ) {
	function dentario_callback_ajax_search() {
		global $post, $wp_query;
		...
		if (!empty($s)) {
			...
			$show_types 	= dentario_get_theme_option('ajax_search_types');
			$show_date 		= dentario_get_theme_option('ajax_search_posts_date')=='yes' ? 1 : 0;
			$show_image 	= dentario_get_theme_option('ajax_search_posts_image')=='yes' ? 1 : 0;
			$show_author 	= dentario_get_theme_option('ajax_search_posts_author')=='yes' ? 1 : 0;
			$show_counters	= dentario_get_theme_option('ajax_search_posts_counters')=='yes' ? dentario_get_theme_option('blog_counters') : '';
			$post_rating	= 'dentario_reviews_avg'.(dentario_get_theme_option('reviews_first')=='author' ? '' : '2');

			$args = array(
				'post_status' => 'publish',
				'orderby' => 'date',
				'order' => 'desc', 
				'posts_per_page' => max(1, min(10, dentario_get_theme_option('ajax_search_posts_count'))),
				's' => esc_html($s),
				);
			// Filter post types
			if (!empty($show_types)) $args['post_type'] = explode(',', $show_types);
			// Exclude categories
			if (dentario_strpos($show_types, 'post') !== false) {
				$ex = dentario_get_theme_option('exclude_cats');
				if (!empty($ex))
			...
			while ( have_posts() ) { the_post();
				$post_number++;
				dentario_template_set_args('widgets-posts', array(
				...
				get_template_part(dentario_get_file_slug('templates/_parts/widgets-posts.php'));
				$output .= dentario_storage_get('widgets_posts_output');
			}
			if (empty($output)) {
				$output .= '<article class="post_item">' . esc_html__('Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'dentario') . '</article>';
			} else {
				$output .= '<article class="post_item"><a href="#" class="post_more search_more">' . esc_html__('More results &hellip;', 'dentario') . '</a></article>';
			}
			$response['data'] = $output;
		} else {
			$response['error'] = esc_html__('The query string is empty!', 'dentario');
		}
...
if (!function_exists('dentario_get_calendar')) {
	function dentario_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		$prev = apply_filters('dentario_filter_calendar_get_prev_month', array(), array(
		...
		if ( $prev_year+$prev_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($prev_year) . '" data-month="' . esc_attr($prev_month) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'dentario'), $wp_locale->get_month($prev_month), date('Y', mktime(0, 0, 0, $prev_month, 1, $prev_year)))) . '">'
		...
		// Get days with posts
		$posts = apply_filters('dentario_filter_calendar_get_curr_month_posts', array(), array(
			'posts_types' => $posts_types,
			'year' => $thisyear,
			'month' => $thismonth,
			'last_day' => $last_day
		...
		if (count($posts) > (!empty($posts['done']) ? 1 : 0)) {
			...
			$link = apply_filters('dentario_filter_calendar_get_month_link', '', array(
				'posts_types' => $posts_types,
				'year' => $thisyear,
				'month' => $thismonth,
				'last_day' => $last_day
			...
			$link = '';
		$calendar_output .= '<th class="month_cur" colspan="5">' . ($link ? '<a href="' . esc_url($link) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'dentario'), $wp_locale->get_month($thismonth), date('Y', mktime(0, 0, 0, $thismonth, 1, $thisyear)))) . '">' : '') . sprintf($calendar_caption, $wp_locale->get_month($thismonth), '<span>'.date('Y', $unixmonth).'</span>') . ($link ? '</a>' : '') . '</th>';

		// Get the next month and year with at least one post
		$next = apply_filters('dentario_filter_calendar_get_next_month', array(), array(
			'posts_types' => $posts_types,
			'year' => $thisyear,
			'month' => $thismonth,
			'last_day' => $last_day
		...
		if ( $next_year+$next_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($next_year) . '" data-month="' . esc_attr($next_month) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'dentario'), $wp_locale->get_month($next_month), date('Y', mktime(0, 0 , 0, $next_month, 1, $next_year))) ) . '">' 
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( !empty($posts[$day]) )
				$calendar_output .= '<a class="day_wrap" href="' . esc_url(!empty($posts[$day]['link']) ? $posts[$day]['link'] : get_day_link($thisyear, $thismonth, $day)) . '" title="' . esc_attr( is_int($posts[$day]['titles']) ? $posts[$day]['titles'].' '.esc_html__('items', 'dentario') : $posts[$day]['titles'] ) . "\">$day</a>";
			else
...
if ( !function_exists( 'dentario_callback_calendar_change_month' ) ) {
	function dentario_callback_calendar_change_month() {
		...
		$response = array('error'=>'', 'data'=>dentario_get_calendar(false, $m, $y, array('post_type'=>$pt)));
...
if (!function_exists('dentario_output_single_comment')) {
	function dentario_output_single_comment( $comment, $args, $depth ) {
		$GLOBALS['comment'] = $comment;
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php esc_html_e( 'Trackback:', 'dentario' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'dentario' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php esc_html_e( 'Pingback:', 'dentario' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'dentario' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					...
					<div class="comment_content">
						<div class="comment_info">
							...
							<span class="comment_date"><span class="comment_date_label"><?php esc_html_e('Posted', 'dentario'); ?></span> <span class="comment_date_value"><?php echo get_comment_date(get_option('date_format')); ?></span></span>
							<span class="comment_time"><?php echo get_comment_date(get_option('time_format')); ?></span>
						</div>
						<div class="comment_text_wrap">
							<?php if ( $comment->comment_approved == 0 ) { ?>
							<div class="comment_not_approved"><?php esc_html_e( 'Your comment is awaiting moderation.', 'dentario' ); ?></div>
							<?php } ?>
...
if (!function_exists('dentario_get_template_page_id')) {
	function dentario_get_template_page_id($name) {
...
if (!function_exists('dentario_detect_template_page_id')) {
	function dentario_detect_template_page_id($key='') {
		static $template_id = '';
		if (!empty($template_id)) return $template_id;
		if (empty($key)) $key = dentario_detect_inheritance_key();
		$tmp = '';
		if (!empty($key)) {
			$inheritance = dentario_get_theme_inheritance($key);
			if (is_singular() && !dentario_storage_get('blog_streampage') && dentario_strpos(dentario_storage_get('page_template'), 'blog-')===false && !empty($inheritance['single_template'])) {
				$tmp = dentario_get_template_page_id($inheritance['single_template']);
			}
			if ((!is_singular() || !$tmp) && !empty($inheritance['stream_template'])) {
				$tmp = dentario_get_template_page_id($inheritance['stream_template']);
			}
		}
		if (empty($tmp)) $tmp = apply_filters('dentario_filter_detect_template_page_id', 0, $key);
		if (dentario_storage_empty('pre_query')) $template_id = $tmp;
		return $tmp;
	}
}
	
// Detect current mode template slug
if (!function_exists('dentario_detect_template_slug')) {
	function dentario_detect_template_slug($key='') {
		...
		if (!empty($key)) {
			$inheritance = dentario_get_theme_inheritance($key);
			if (is_singular() && !dentario_storage_get('blog_streampage') && dentario_strpos(dentario_storage_get('page_template'), 'blog-')===false && !empty($inheritance['single_template'])) {
				$tmp = dentario_get_slug($inheritance['single_template']);
			}
			if ((!is_singular() || !$tmp) && !empty($inheritance['stream_template'])) {
				$tmp = dentario_get_slug($inheritance['stream_template']);
			}
		}
		if (empty($tmp)) $tmp = apply_filters('dentario_filter_detect_template_slug', '', $key);
		if (empty($tmp)) $tmp = $key;
		if (dentario_storage_empty('pre_query')) $template_slug = $tmp;
		return $tmp;
	}
}
	
// Return slug for the specified template
if (!function_exists('dentario_get_template_slug')) {
	function dentario_get_template_slug($key) {
		$template_slug = '';
		$inheritance = dentario_get_theme_inheritance($key);
		if (is_singular() && !dentario_storage_get('blog_streampage') && !empty($inheritance['single_template'])) {
			$template_slug = dentario_get_slug($inheritance['single_template']);
		}
		if ((!is_singular() || !$template_slug) && !empty($inheritance['stream_template'])) {
			$template_slug = dentario_get_slug($inheritance['stream_template']);
		}
...
if (!function_exists('dentario_get_stream_page_id')) {
	function dentario_get_stream_page_id($mode) {
		return apply_filters('dentario_filter_get_stream_page_id', '', $mode);
	}
}
	
// Return link to the stream page for specified mode
if (!function_exists('dentario_get_stream_page_link')) {
	function dentario_get_stream_page_link($mode) {
		return apply_filters('dentario_filter_get_stream_page_link', '', $mode);
	}
}
	
// Return title of the stream page for specified mode
if (!function_exists('dentario_get_stream_page_title')) {
	function dentario_get_stream_page_title($mode) {
		return apply_filters('dentario_filter_get_stream_page_title', '', $mode);
	}
}

// Return taxonomy name (slug) if current page is this taxonomy page
if ( !function_exists( 'dentario_is_taxonomy' ) ) {
	function dentario_is_taxonomy($query=null) {
		static $tax = '';
		if (!empty($tax)) return $tax;
		$tax = apply_filters('dentario_filter_is_taxonomy',	$tax);
		if (!empty($tax)) return $tax;
...
if ( !function_exists( 'dentario_get_current_term' ) ) {
	function dentario_get_current_term($tax='') {
		static $term = null;
...
if (!function_exists('dentario_get_terms_by_post_id')) {
	function dentario_get_terms_by_post_id($args=array()) {
		$args = array_merge(array(
			'post_id' 	=> 0,
			'taxonomy' 	=> array(),
			'parent_id'	=> 0,
		...
		$closest_method = dentario_get_theme_option('close_category');
		// make terms list
		$terms_list = array();
				...
				if (!empty($terms) && is_array($terms)) {
					...
					foreach ( $terms as $term ) {
						...
						if ($taxonomy_obj->hierarchical && empty($terms_list[$taxonomy_slug]->closest_parent)) {
							$terms_list[$taxonomy_slug]->closest_parent = $closest_method == 'parental' ? dentario_get_parent_taxonomy($term->term_id, $args['parent_id'], $taxonomy_slug) : $term;
...
if (!function_exists('dentario_get_categories_by_post_id')) {
	function dentario_get_categories_by_post_id($args = array()) {
		if (!is_array($args)) $args = array('post_id' => $args);
		if (!isset($args['taxonomy'])) $args['taxonomy'] = 'category';
		return dentario_get_terms_by_post_id($args);
	}
}

// Return tags objects by post id
if (!function_exists('dentario_get_tags_by_post_id')) {
	function dentario_get_tags_by_post_id($args = array()) {
		if (!is_array($args)) $args = array('post_id' => $args);
...
if (!function_exists('dentario_get_terms_by_taxonomy')) {
	function dentario_get_terms_by_taxonomy($tax_types = 'post_format', $args=array()) {
...
if (!function_exists('dentario_get_terms_by_taxonomy_from_db')) {
	function dentario_get_terms_by_taxonomy_from_db($tax_types = 'post_format') {
		global $wpdb;
		if (!is_array($tax_types)) $tax_types = array($tax_types);
...
if (!function_exists('dentario_get_parent_taxonomy')) {
	function dentario_get_parent_taxonomy($id, $parent_id=0, $taxonomy='category') {
		$val = null;
		do {
...
if (!function_exists('dentario_get_parent_category')) {
	function dentario_get_parent_category($id, $parent_id=0) {
		return dentario_get_parent_taxonomy($id, $parent_id, 'category');
	}
}

// Return id nearest (or highest) parent taxonomy with specified property in array values
if (!function_exists('dentario_get_parent_taxonomy_by_property')) {
	function dentario_get_parent_taxonomy_by_property($id, $prop, $values, $highest=true, $taxonomy='category') {
		if ((int) $id == 0) {
		...
		$val = $id;
		do {
			if ($props = dentario_taxonomy_load_custom_options($id, $taxonomy)) {
...
if (!function_exists('dentario_get_parent_category_by_property')) {
	function dentario_get_parent_category_by_property($id, $prop, $values, $highest=true) {
		return dentario_get_parent_taxonomy_by_property($id, $prop, $values, $highest, 'category');
	}
}

// Show categories, tags, post-formats filters for posts and attachments
if ( !function_exists( 'dentario_show_taxonomies_filters' ) ) {
	function dentario_show_taxonomies_filters() {
		if (dentario_get_theme_option('admin_add_filters')!='yes') return;
		...
		if ($page == 'post')
			...
			return;
		echo trim(dentario_get_terms_filters($taxes));
	}
}

// Return string with <select> tags for each taxonomy
if (!function_exists('dentario_get_terms_filters')) {
	function dentario_get_terms_filters($taxes) {
		$output = '';
		if (is_array($taxes) && count($taxes) > 0) {
			foreach ($taxes as $tax) {
				$list = get_transient("dentario_terms_filter_".($tax));
				if (!$list) {
					$list = '';
					$tax_obj = get_taxonomy($tax);
					$terms = dentario_get_terms_hierarchical_list(dentario_get_terms_by_taxonomy($tax));
					if (is_array($terms) && count($terms) > 0) {
				...
				set_transient("dentario_terms_filter_".($tax), $list, 0);
				$output .= $list;
			}
		}
		return $output;
...
if (!function_exists('dentario_get_terms_hierarchical_list')) {
	function dentario_get_terms_hierarchical_list($terms, $opt=array()) {
		...
		if (is_array($terms) && count($terms) > 0) {
			foreach ($terms as $term) {
				...
				$rez = array_merge($rez, dentario_get_terms_hierarchical_list($terms, array(
...
if ( !function_exists( 'dentario_callback_clear_cache' ) ) {
	function dentario_callback_clear_cache() {
...
if (!function_exists('dentario_clear_cache')) {
	function dentario_clear_cache($cc) {
...
if ( !function_exists( 'dentario_clear_cache_all' ) ) {
	function dentario_clear_cache_all($post_id=0, $post_obj=null) {
		delete_transient("dentario_terms_filter_category");
		delete_transient("dentario_terms_filter_post_tag");
		delete_transient("dentario_terms_filter_post_format");
	}
}

// Clear categories cache (when create, edit or delete category)
if ( !function_exists( 'dentario_clear_cache_categories' ) ) {
	function dentario_clear_cache_categories($term_id=0) {
		delete_transient("dentario_terms_filter_category");
	}
}
...
if (!function_exists('dentario_get_post_data_options')) {
	function dentario_get_post_data_options($opt) {
		$opt = array_merge(array(
			...
			'counters' => dentario_get_custom_option("blog_counters"),	// What counters use: views or comments
			...
			'sidebar' => !dentario_param_is_off(dentario_get_custom_option('show_sidebar_main')),	// Sidebar is visible on current page
			'readmore' => null,			// Text for the "Read more" link
			'strip_teaser' => dentario_get_custom_option('show_text_before_readmore')!='yes',	// Strip text before <!--more--> tag in the content or show full content
			'substitute_gallery' => dentario_get_custom_option('substitute_gallery')=='yes',	// Substitute standard WP gallery on theme slider
			'substitute_video'   => dentario_get_custom_option('substitute_video')=='yes',		// Substitute tag <video> on the Youtube or Vimeo player
			'substitute_audio'   => dentario_get_custom_option('substitute_audio')=='yes',		// Substitute tag <audio> on the Sound Cloud's player
...
if (!function_exists('dentario_show_post_layout')) {
	function dentario_show_post_layout($opt = array(), $post_data=null, $post_obj=null) {
		$opt = dentario_get_post_data_options($opt);
		// Collect standard output
		$layout = '';
		$func_name = dentario_get_template_function_name($opt['layout']);
		if (function_exists($func_name)) {
			if (!$opt['show']) ob_start();
			if ($post_data === null) {
				$post_data = dentario_get_post_data($opt, $post_obj);
			}
			...
			dentario_clear_dedicated_content();
		}
		return $layout;
	}
}
...
if (!function_exists('dentario_get_post_data')) {
	function dentario_get_post_data(&$opt, $post_obj=null) {
		$opt = dentario_get_post_data_options($opt);
		global $post, $wp_query;
		...
		//if (dentario_exists_wpml() && !empty($post) && is_object($post)) $cur_post = clone $post;
		$post_id = get_the_ID();
		$cached_post = dentario_storage_get('post_data_'.$post_id);		// Restore cached post data
		if (is_array($cached_post)) {
			...
			$post_icon = dentario_get_custom_option('icon', dentario_get_post_format_icon($post_format), $post_id, $post_type);	//!!!!! Get option from specified post
			$post_flags = array(
				'sticky' => is_sticky()
			);
		...
		if (!empty($opt['date_format'])) {
			...
			if ( ($post_date_part1) . ($post_date_part2) != '' ) {
				$post_date = dentario_get_date_translations($post_date_part1 . (!empty($post_date_part2) ? ' '.trim($post_date_part2) : ''));
			}
		}
		...
		if ($opt['counters']!='') {
			if (is_array($cached_post) && $cached_post['post_options_counters']!='') {
				...
				$post_views = dentario_get_post_views($post_id);
				$post_likes = dentario_get_post_likes($post_id);
			}
		}
		...
		if ($opt['reviews']) {
			if (is_array($cached_post) && $cached_post['post_options_reviews']) {
				...
				$post_reviews_author = dentario_reviews_marks_to_display(get_post_meta($post_id, 'dentario_reviews_avg', true));
				$post_reviews_users  = dentario_reviews_marks_to_display(get_post_meta($post_id, 'dentario_reviews_avg2', true));
			}
		}
		...
		if (is_array($cached_post)) {
			...
			$allow_editor = dentario_get_theme_option("allow_editor")=='yes';
		...
		if ($opt['content']) {
			if (!empty($post_content) && $opt['substitute_gallery'] && !dentario_in_shortcode_blogger(true))	$post_content = dentario_substitute_gallery($post_content, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop'], true);
			...
			*/
			...
			if (!empty($post_content)) {
				if ($opt['substitute_video'] && !dentario_in_shortcode_blogger(true)) 	$post_content = dentario_substitute_video($post_content, $thumb_sizes['w'], $thumb_sizes['h_crop']);
				if ($opt['substitute_audio'] && !dentario_in_shortcode_blogger(true))	$post_content = dentario_substitute_audio($post_content);
			}
		}
		...
		if (is_array($cached_post)) {
			...
			if (empty($post_excerpt)) {
				if (($more_pos = dentario_strpos($post_content_plain, '<span id="more-'))!==false) {
					$post_excerpt = dentario_substr($post_content_plain, 0, $more_pos);
				} else {
					$post_excerpt = in_array($post_format, array('quote', 'link')) ? $post_content : strip_shortcodes(strip_tags(get_the_excerpt()));
				}
			}
			if (!empty($post_excerpt) && $opt['substitute_gallery'] && !dentario_in_shortcode_blogger(true)) $post_excerpt = dentario_substitute_gallery($post_excerpt, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop']);
			if (!empty($post_excerpt)) $post_excerpt = apply_filters('dentario_filter_sc_clear_around', $post_excerpt);
			$post_excerpt = apply_filters('the_excerpt', $post_excerpt);
			if (!empty($post_excerpt)) {
				$post_excerpt = apply_filters('dentario_filter_p_clear_around', $post_excerpt);
				/*
				// Restore post from clone
				if ($post_id != get_the_ID() && $cur_post!==null) {		// Fix bug in the WPML
					$post = clone $cur_post;
				...
				*/
				...
				if ($opt['substitute_video'] && !dentario_in_shortcode_blogger(true)) $post_excerpt = dentario_substitute_video($post_excerpt, $thumb_sizes['w'], $thumb_sizes['h_crop']);
				if ($opt['substitute_audio'] && !dentario_in_shortcode_blogger(true)) $post_excerpt = dentario_substitute_audio($post_excerpt);
		...
		$opt['dedicated'] = dentario_get_dedicated_content();
		$opt['location']  = !empty($opt['location']) ? $opt['location'] : dentario_get_custom_option('dedicated_location');
		if (empty($opt['location']) || $opt['location'] == 'default')
			$opt['location'] = dentario_get_custom_option('dedicated_location', '', $post_id, $post_type);	//!!!!! Get option from specified post
		...
		if (!empty($opt['dedicated'])) {
			$class = dentario_get_tag_attrib($opt['dedicated'], '<div class="sc_section>', 'class');
			if ($opt['location']=='default') {
				if (($pos = dentario_strpos($class, 'sc_align'))!==false) {
					$pos += 8;
					$pos2 = dentario_strpos($class, ' ', $pos);
					$opt['location'] = $pos2===false ? dentario_substr($class, $pos) : dentario_substr($class, $pos, $pos2-$pos);
				}
				if ($opt['location']=='' || $opt['location']=='default') $opt['location'] = 'center';
			}
			if (!is_singular() || dentario_storage_get('blog_streampage') || dentario_in_shortcode_blogger(true) || (dentario_strpos($class, 'sc_align')!==false && dentario_strpos($class, 'columns')===false)) {
				$class = str_replace(array('sc_alignright', 'sc_alignleft', 'sc_aligncenter'), array('','',''), $class) . ' sc_align' . esc_attr($opt['location']);
				$opt['dedicated'] = dentario_set_tag_attrib($opt['dedicated'], '<div class="sc_section>', 'class', $class);
			}
		}
		$opt['post_class'] = $opt['location'];
		// Substitute <video> tags to <iframe> in dedicated content
		if ($opt['substitute_video'] && !dentario_in_shortcode_blogger(true)) {
			$opt['dedicated'] = dentario_substitute_video($opt['dedicated'], $thumb_sizes['w'], $thumb_sizes['h_crop']);
		}
		// Substitute <audio> tags with src from soundcloud to <iframe>
		if ($opt['substitute_audio'] && !dentario_in_shortcode_blogger(true)) {
			$opt['dedicated'] = dentario_substitute_audio($opt['dedicated']);
		}
		...
		if ($post_format == 'gallery') {
			...
			if ($post_video=='') {
				...
				if (!dentario_is_youtube_url($src) && !dentario_is_vimeo_url($src))
					$src = '';
			} else {
			...
			if ($post_video!='' && $opt['substitute_video']) {	// && !dentario_in_shortcode_blogger(true)) {
				$post_video = dentario_substitute_video($post_video, $thumb_sizes['w'], round($thumb_sizes['w']/16*9), false);	//$thumb_sizes['h_crop']);
			}
			...
			if ($post_audio!='' && $opt['substitute_audio']) {	// && !dentario_in_shortcode_blogger(true)) {
		...
		if (is_array($cached_post) && $cached_post['post_options_terms_list']) {
			...
			$post_taxonomy = dentario_get_taxonomy_categories_by_post_type($post_type);
			$post_taxonomy_tags = dentario_get_taxonomy_tags_by_post_type($post_type);
		}
		$post_terms = array();
		if ($opt['terms_list']) {
			if (is_array($cached_post) && $cached_post['post_options_terms_list']) {
				...
				$post_terms = dentario_get_terms_by_post_id( array(
					'post_id' 	=> $post_id,
					'parent_id'	=> $post_type=='post' ? $opt['parent_tax_id'] : 0,
					'exclude'	=> $post_type=='post' ? explode(',', dentario_get_theme_option('exclude_cats')) : array()
					)
				);
			}
		}
		...
		if (dentario_get_theme_setting('use_post_cache') && !dentario_storage_isset('post_data_'.$post_id)) dentario_storage_set('post_data_'.$post_id, $post_data);

		return apply_filters('dentario_filter_get_post_data', $post_data, $opt, $post_obj);
	}
}

// Return custom_page_heading (if set), else - post title
if (!function_exists('dentario_get_post_title')) {
	function dentario_get_post_title($id = 0, $maxlength = 0, $add='...') {
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$title = get_the_title($id);
		if ($maxlength > 0) $title = dentario_strshort($title, $maxlength, $add);
		return $title;
	}
}

// Return post excerpt
if (!function_exists('dentario_get_post_excerpt')) {
	function dentario_get_post_excerpt($maxlength = 0, $add='...') {
		$descr = get_the_excerpt();
		$descr = trim(str_replace(array('[...]', '[&hellip;]'), array($add, $add), $descr));
		if (!empty($descr) && dentario_strpos(',.:;-', dentario_substr($descr, -1))!==false) $descr = dentario_substr($descr, 0, -1);
		if ($maxlength > 0) $descr = dentario_strshort($descr, $maxlength, $add);
		return $descr;
	}
}

//Return Post Views Count
if (!function_exists('dentario_get_post_views')) {
	function dentario_get_post_views($id=0){
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'dentario_post_views_count';
...
if (!function_exists('dentario_set_post_views')) {
	function dentario_set_post_views($id=0, $counter=-1) {
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'dentario_post_views_count';
		$count = get_post_meta($id, $count_key, true);
...
if (!function_exists('dentario_get_post_likes')) {
	function dentario_get_post_likes($id=0){
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'dentario_post_likes_count';
		$count = get_post_meta($id, $count_key, true);
		if ($count===''){
			delete_post_meta($id, $count_key);
			add_post_meta($id, $count_key, '0');
...
if (!function_exists('dentario_set_post_likes')) {
	function dentario_set_post_likes($id=0, $count=0) {
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'dentario_post_likes_count';
		update_post_meta($id, $count_key, $count);
...
// add_action('wp_ajax_post_counter', 			'dentario_callback_post_counter');
// add_action('wp_ajax_nopriv_post_counter',	'dentario_callback_post_counter');
if ( !function_exists( 'dentario_callback_post_counter' ) ) {
	function dentario_callback_post_counter() {
		...
		if (isset($_REQUEST['likes'])) {
			...
			dentario_set_post_likes($id, $counter);
		} else if (isset($_REQUEST['views'])) {
			$counter = max(0, (int) $_REQUEST['views']);
			dentario_set_post_views($id, $counter);
		}
		echo json_encode($response);
		die();
	}
...
// Pre query: Add filter to main query
if ( !function_exists( 'dentario_query_posts_where' ) ) {
	// add_filter('posts_where', 'dentario_query_posts_where', 10, 2);  
	function dentario_query_posts_where($where, $query) { 
		global $wpdb; 
		if (is_admin() || $query->is_attachment) return $where;
		if (dentario_strpos($where, 'post_status')===false && (!isset($_REQUEST['preview']) || $_REQUEST['preview']!='true') && (!isset($_REQUEST['vc_editable']) || $_REQUEST['vc_editable']!='true')) {
...
if ( !function_exists( 'dentario_query_add_posts_per_page' ) ) {
	// add_action( 'pre_get_posts', 'dentario_query_add_posts_per_page' );
	function dentario_query_add_posts_per_page($query) {

		if (is_admin() || !$query->is_main_query()) return;

		// Set prequery mode
		dentario_storage_set('pre_query', $query);
		...
		if ($id > 0)  {
			...
			dentario_storage_set('page_template', $slug[0]);
		}

		if (!is_singular() || $query->is_posts_page==1 || dentario_strpos(dentario_storage_get('page_template'), 'blog-')!==false) {
			$orderby = $order = $ppp = '';
	
			// Check template page settings
			$orderby_set = apply_filters('dentario_filter_orderby_need', $query->is_archive() || $query->is_search() || $query->is_posts_page==1 || dentario_strpos(dentario_storage_get('page_template'), 'blog-')!==false);
			...
			if ( !empty($slug) ) {
				if (empty($inheritance['use_options_page']) || $inheritance['use_options_page'])
					$template_options = get_option( dentario_storage_get('options_prefix') . '_options_template_'.trim($slug) );
				else
				...
				if ($template_options===false && !empty($inheritance['override'])) {
					...
					if ( !empty($slug) ) $template_options = get_option( dentario_storage_get('options_prefix') . '_options_template_'.trim($slug));
				}
				if ($template_options!==false) {
					if (isset($template_options['posts_per_page']) && !empty($template_options['posts_per_page']) && !dentario_is_inherit_option($template_options['posts_per_page'])) {
						$ppp = (int) $template_options['posts_per_page'];
					}
					if ($orderby_set) {
						if (isset($template_options['blog_sort']) && !empty($template_options['blog_sort']) && !dentario_is_inherit_option($template_options['blog_sort']))
							$orderby = $template_options['blog_sort'];
						if (isset($template_options['blog_order']) && !empty($template_options['blog_order']) && !dentario_is_inherit_option($template_options['blog_order']))
			...
			if ( !empty($query->tax_query->queried_terms) && is_array($query->tax_query->queried_terms) ) {
				foreach($query->tax_query->queried_terms as $tax=>$terms) {
					if (!empty($terms['terms'][0])) {
						$term = $terms['terms'][0];
						$tax_options = dentario_taxonomy_get_inherited_properties($tax, $term);
						if (empty($ppp) && isset($tax_options['posts_per_page']) && !empty($tax_options['posts_per_page']) && !dentario_is_inherit_option($tax_options['posts_per_page'])) {
							$ppp = (int) $tax_options['posts_per_page'];
						}
						if ($orderby_set) {
							if (isset($tax_options['blog_sort']) && !empty($tax_options['blog_sort']) && !dentario_is_inherit_option($tax_options['blog_sort']))
								$orderby = $tax_options['blog_sort'];
							if (isset($tax_options['blog_order']) && !empty($tax_options['blog_order']) && !dentario_is_inherit_option($tax_options['blog_order']))
			...
			if (empty($ppp))
				$ppp = (int) dentario_get_theme_option('posts_per_page');
			if ($ppp > 0)
				$query->set('posts_per_page', $ppp );
	
			if ($orderby_set) {
				if (empty($orderby))
					$orderby = dentario_get_theme_option('blog_sort');
				if (empty($order))
					$order = dentario_get_theme_option('blog_order');
				 dentario_query_add_sort_order($query, $orderby, $order);
			}
			// Exclude categories
			$ex = dentario_get_theme_option('exclude_cats');
			if (!empty($ex))
		...
		// Reset prequery mode
		dentario_storage_set('pre_query', null);
	}
}

// Filter categories list - exclude unwanted cats from widget output
if ( !function_exists( 'dentario_query_exclude_categories' ) ) {
	// add_action( 'widget_categories_args', 			'dentario_query_exclude_categories' );
	// add_action( 'widget_categories_dropdown_args',	'dentario_query_exclude_categories' );
	function dentario_query_exclude_categories($args) {
		if (!is_admin()) {
			$ex = dentario_get_theme_option('exclude_cats');
			if (!empty($ex))
				$args['exclude'] = $ex;
...
if ( !function_exists( 'dentario_query_exclude_posts' ) ) {
	//add_action( 'widget_posts_args', 'dentario_query_exclude_posts' );
	function dentario_query_exclude_posts($args) {
		if (!is_admin()) {
			$ex = dentario_get_theme_option('exclude_cats');
			if (!empty($ex)) {
				$args['category__not_in'] = explode(',', $ex);
...
if ( !function_exists( 'dentario_query_exclude_archives_where' ) ) {
	// add_action( 'getarchives_where', 'dentario_query_exclude_archives_where', 10, 2 );
	function dentario_query_exclude_archives_where($where, $r='') {
		if (!is_admin()) {
			$ex = dentario_get_theme_option('exclude_cats');
			if (!empty($ex)) {
				global $wpdb;
...
if (!function_exists('dentario_query_add_sort_order')) {
	function dentario_query_add_sort_order($args, $orderby='', $order='') {
		if (empty($order)) $order = dentario_get_custom_option('blog_order');
		if (empty($orderby)) $orderby = dentario_get_custom_option('blog_sort');
		$q = array();
		$q['order'] = $order=='asc' ? 'asc' : 'desc';
		if ($orderby == 'author_rating') {
			$q['orderby'] = 'meta_value_num';
			$q['meta_key'] = 'dentario_reviews_avg';
			$q['meta_query'] = array(
				array(
					'meta_filter' => 'reviews',
					'key' => 'dentario_reviews_avg',
					'value' => 0,
					'compare' => '>=',
					'type' => 'NUMERIC'
				   )
			...
			$q['orderby'] = 'meta_value_num';
			$q['meta_key'] = 'dentario_reviews_avg2';
			$q['meta_query'] = array(
				array(
					'meta_filter' => 'reviews',
					'key' => 'dentario_reviews_avg2',
					'value' => 0,
					'compare' => '>=',
					'type' => 'NUMERIC'
				   )
			...
			$q['orderby'] = 'meta_value_num';
			$q['meta_key'] = 'dentario_post_views_count';
		...
		$q = apply_filters('dentario_filter_add_sort_order', $q, $orderby, $order);
		foreach ($q as $mk=>$mv) {
			if (is_array($args))
				$args[$mk] = $mv;
			else
...
if (!function_exists('dentario_query_add_posts_and_cats')) {
	function dentario_query_add_posts_and_cats($args, $ids='', $post_type='', $cat='', $taxonomy='') {
		if (!empty($ids)) {
...
if (!function_exists('dentario_query_add_filters')) {
	function dentario_query_add_filters($args, $filters=false) {
		if (!empty($filters)) {
				...
				if (in_array($v, array('reviews', 'thumbs'))) {							// Filter with meta_query
					...
					if (!$found) {
						...
						if ($v == 'reviews') {
							$args['meta_query'][] = array(
								'meta_filter' => $v,
								'key' => 'dentario_reviews_avg'.(dentario_get_theme_option('reviews_first')=='author' ? '' : '2'),
								'value' => 0,
								'compare' => '>',
								'type' => 'NUMERIC'
							);
					...
					$args = apply_filters('dentario_filter_query_add_filters', $args, $v);
			}
		}
...
// For example: if (dentario_check_user_role('author')) { ... }
if (!function_exists('dentario_check_user_role')) {
	function dentario_check_user_role( $role, $user_id = null ) {
...
// AJAX: New user registration
if ( !function_exists( 'dentario_callback_registration_user' ) ) {
	// add_action('wp_ajax_registration_user',			'dentario_callback_registration_user');
	// add_action('wp_ajax_nopriv_registration_user',	'dentario_callback_registration_user');
	function dentario_callback_registration_user() {
	
		if ( !wp_verify_nonce( dentario_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			die();
	
		$user_name  = dentario_substr($_REQUEST['user_name'], 0, 20);
		$user_email = dentario_substr($_REQUEST['user_email'], 0, 60);
		$user_pwd   = dentario_substr($_REQUEST['user_pwd'], 0, 20);
		...
		if ( is_wp_error($id) ) {
		...
		} else if (($notify = dentario_get_theme_option('notify_about_new_registration'))!='no' && (($contact_email = dentario_get_theme_option('contact_email')) || ($contact_email = dentario_get_theme_option('admin_email')))) {
			$mail = dentario_get_theme_option('mail_function');
			if (in_array($notify, array('both', 'admin', 'yes'))) {
				$subj = sprintf(esc_html__('Site %s - New user registration: %s', 'dentario'), esc_html(get_bloginfo('site_name')), esc_html($user_name));
				$msg = "\n".esc_html__('New registration:', 'dentario')
					."\n".esc_html__('Name:', 'dentario').' '.esc_html($user_name)
					."\n".esc_html__('E-mail:', 'dentario').' '.esc_html($user_email)
			...
			if (in_array($notify, array('both', 'user', 'yes'))) {
				$subj = sprintf(esc_html__('Welcome to "%s"', 'dentario'), get_bloginfo('site_name'));
				$msg = "\n".esc_html__('Your registration data:', 'dentario')
					."\n".esc_html__('Name:', 'dentario').' '.esc_html($user_name)
					."\n".esc_html__('E-mail:', 'dentario').' '.esc_html($user_email)
					."\n".esc_html__('Password:', 'dentario').' '.esc_html($user_pwd)
...
// AJAX: Login user
if ( !function_exists( 'dentario_callback_login_user' ) ) {
	// add_action('wp_ajax_login_user',			'dentario_callback_login_user');
	// add_action('wp_ajax_nopriv_login_user',	'dentario_callback_login_user');
	function dentario_callback_login_user() {
	
		if ( !wp_verify_nonce( dentario_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			die();
	
		$user_log = dentario_substr($_REQUEST['user_log'], 0, 60);
		$user_pwd = dentario_substr($_REQUEST['user_pwd'], 0, 20);
		$remember = dentario_substr($_REQUEST['remember'], 0, 7)=='forever';
...
if ( !function_exists( 'dentario_allow_email_login' ) ) {
	function dentario_allow_email_login( $user, $username, $password ) {
...
/* Nav menu utils
------------------------------------------------------------------------------------- */
if ( !function_exists( 'dentario_get_nav_menu' ) ) {
	function dentario_get_nav_menu($slug='', $custom_walker=false, $mobile='') {
		$menu = !empty($slug) ? dentario_get_custom_option($slug) : '';
		$menu = empty($menu) || $menu=='default' || dentario_is_inherit_option($menu) ? '' : $menu;
		$list = dentario_get_theme_option('use_menu_cache')=='yes' ? get_transient('dentario_menu') : array();
		...
		if (empty($html)) {
			...
			if ($custom_walker && class_exists('dentario_custom_menu_walker') && dentario_get_theme_option('custom_menu')=='yes')
				$args['walker'] = new dentario_custom_menu_walker;
			$html = wp_nav_menu(apply_filters('dentario_filter_get_nav_menu', $args));
			if (!empty($slug) && dentario_get_theme_option('use_menu_cache')=='yes') {
				$list[trim($slug).'_'.trim($menu)] = $html;
				set_transient('dentario_menu', $list, 24*60*60);
			}
		}
		return $html;
	}
...
if ( !function_exists( 'dentario_clear_cache_menu' ) ) {
	function dentario_clear_cache_menu($menu_id=0, $menu_data=array()) {
		delete_transient("dentario_menu");
	}
}
...
if (!function_exists('dentario_prepare_widgets_args')) {
	function dentario_prepare_widgets_args($args, $id, $class) {
...
if (!function_exists('dentario_theme_support')) {
	function dentario_theme_support($type, $value, $params) {
		if (function_exists('trx_utils_theme_support'))
			trx_utils_theme_support( $type, $value, $params);
	}
}
if (!function_exists('dentario_theme_support_pt')) {
	function dentario_theme_support_pt($value, $params=false) {
		if (function_exists('trx_utils_theme_support_pt'))
			trx_utils_theme_support_pt($value, $params);
	}
}
if (!function_exists('dentario_theme_support_tx')) {
	function dentario_theme_support_tx($value, $params=false) {
...
if (!function_exists('dentario_require_shortcode')) {
	function dentario_require_shortcode($name, $cb) {
...
if (!function_exists('dentario_do_shortcode')) {
	function dentario_do_shortcode($text) {
		$parts = explode(' ', $text, 2);
		if (dentario_substr($parts[0], 0, 5)!=='[trx_' || shortcode_exists(str_replace(array('[', ']'), array('', ''), $parts[0])))
...
if (!function_exists('dentario_param_is_on')) {
	function dentario_param_is_on($prm) {
		return $prm>0 || in_array(dentario_strtolower($prm), array('true', 'on', 'yes', 'show'));
	}
}
if (!function_exists('dentario_param_is_off')) {
	function dentario_param_is_off($prm) {
		return empty($prm) || $prm===0 || in_array(dentario_strtolower($prm), array('false', 'off', 'no', 'none', 'hide'));
	}
}
if (!function_exists('dentario_param_is_inherit')) {
	function dentario_param_is_inherit($prm) {
		return in_array(dentario_strtolower($prm), array('inherit', 'default'));
	}
}

// Clear dedicated content
if (!function_exists('dentario_clear_dedicated_content')) {
	function dentario_clear_dedicated_content() {
		dentario_storage_set('sc_section_dedicated', '');
	}
}

// Return dedicated content
if (!function_exists('dentario_get_dedicated_content')) {
	function dentario_get_dedicated_content() {
		return dentario_storage_get('sc_section_dedicated');
	}
}

// Check if we are in the shortcode "trx_blogger" now
if (!function_exists('dentario_in_shortcode_blogger')) {
	function dentario_in_shortcode_blogger($from_blogger = false) {
		if (!$from_blogger) return false;
		return dentario_storage_get('sc_blogger_busy')===true;
	}
}

// Return GAP wrapper start
if (!function_exists('dentario_gap_start')) {
	function dentario_gap_start() {
		return '<!-- #TRX_GAP_START# -->';
	}
}

// Return GAP wrapper end
if (!function_exists('dentario_gap_end')) {
	function dentario_gap_end() {
		return '<!-- #TRX_GAP_END# -->';
	}
}
...
if (!function_exists('dentario_gap_wrapper')) {
	function dentario_gap_wrapper($str) {
		// Move VC row and column and wrapper inside gap
		// Old VC wrappers
		$str_new = preg_replace('/(<div\s+class="[^"]*vc_row[^>]*>)[\r\n\s]*(<div\s+class="[^"]*vc_col[^>]*>)[\r\n\s]*(<div\s+class="[^"]*wpb_wrapper[^>]*>)[\r\n\s]*('.dentario_gap_start().')/i', '\\4\\1\\2\\3', $str);
		if ($str_new != $str) $str_new = preg_replace('/('.dentario_gap_end().')[\r\n\s]*(<\/div>)[\r\n\s]*(<\/div>)[\r\n\s]*(<\/div>)/i', '\\2\\3\\4\\1', $str_new);
		// New VC wrappers
		$str_new = preg_replace('/(<div\s+class="[^"]*vc_row[^>]*>)[\r\n\s]*(<div\s+class="[^"]*vc_col[^>]*>)[\r\n\s]*(<div\s+class="[^"]*vc_col[^>]*>)[\r\n\s]*(<div\s+class="[^"]*wpb_wrapper[^>]*>)[\r\n\s]*('.dentario_gap_start().')/i', '\\5\\1\\2\\3\\4', $str);
		...
		return str_replace(
				...
				array(
					dentario_close_all_wrappers(false) . '<div class="sc_gap">',
					'</div>' . dentario_open_all_wrappers(false)
				),
				$str_new
			); 
	}
