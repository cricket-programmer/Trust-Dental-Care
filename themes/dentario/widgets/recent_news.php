...
// Theme init
if (!function_exists('dentario_widget_recent_news_theme_setup')) {
	add_action( 'dentario_action_before_init_theme', 'dentario_widget_recent_news_theme_setup', 1 );
	function dentario_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('dentario_action_shortcodes_list',	'dentario_widget_recent_news_reg_shortcodes');
		if (function_exists('dentario_exists_visual_composer') && dentario_exists_visual_composer())
			add_action('dentario_action_shortcodes_list_vc','dentario_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('dentario_widget_recent_news_load')) {
	add_action( 'widgets_init', 'dentario_widget_recent_news_load' );
	function dentario_widget_recent_news_load() {
		register_widget('dentario_widget_recent_news');
	}
}


// Widget Class
class dentario_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'dentario'));
		parent::__construct( 'dentario_widget_recent_news', esc_html__('Dentario - Recent News', 'dentario'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = dentario_sc_recent_news( array(
			'from_widget'		=> true,
	...
	function form($instance) {
		...
		$news_styles = dentario_get_list_templates('news');
		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'dentario'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'dentario'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'dentario'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'dentario'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed:', 'dentario'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'dentario'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'dentario'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'dentario'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'dentario'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'dentario'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show counters:', 'dentario'); ?></label><br />
...
*/
if ( !function_exists( 'dentario_sc_recent_news' ) ) {
	function dentario_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (dentario_param_is_on($show_categories)) {
			if ( !dentario_storage_isset('categories_'.$category) ) {
				dentario_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = dentario_prepare_widgets_args(dentario_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (dentario_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (dentario_param_is_on($show_categories) && !dentario_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(dentario_param_is_on($show_categories) && !dentario_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = dentario_storage_get('categories_'.$category);
			if (dentario_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'dentario').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cat_list as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'dentario')
		...
		$args = dentario_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('dentario_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	dentario_require_shortcode("trx_recent_news", "dentario_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('dentario_widget_recent_news_reg_shortcodes_vc')) {
	//add_action('dentario_action_shortcodes_list_vc','dentario_widget_recent_news_reg_shortcodes_vc');
	function dentario_widget_recent_news_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Recent News", 'dentario'),
				"description" => wp_kses_data( __("Insert recent news list", 'dentario') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("List style", 'dentario'),
						"description" => wp_kses_data( __("Select style to display news list", 'dentario') ),
						"class" => "",
						"admin_label" => true,
						"std" => 'news-magazine',
					...
					array(
						...
						"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", 'dentario') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"heading" => esc_html__("Show categories", 'dentario'),
						"description" => wp_kses_data( __("Show categories in the shortcode's header", 'dentario') ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show counters", 'dentario'),
						"description" => wp_kses_data( __("Show counters (comments and views) in the shortcode's footer", 'dentario') ),
						'dependency' => array(
							'element' => 'style',
							'value' => 'news-magazine'
						),
					...
					array(
						...
						"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", 'dentario') ),
						"group" => esc_html__('Query', 'dentario'),
						"class" => "",
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", 'dentario'),
					...
					array(
						"param_name" => "featured",
						"heading" => esc_html__("Featured posts", 'dentario'),
						"description" => wp_kses_data( __("How many posts will be displayed as featured?", 'dentario') ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'dentario'),
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"description" => wp_kses_data( __("How many columns use to show news list", 'dentario') ),
						"group" => esc_html__('Query', 'dentario'),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"group" => esc_html__('Query', 'dentario'),
						"class" => "",
						"value" => array_flip(dentario_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'dentario'),
						"class" => "",
						"value" => array_flip(dentario_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
						"heading" => esc_html__("List IDs", 'dentario'),
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", 'dentario') ),
						"group" => esc_html__('Query', 'dentario'),
						"class" => "",
						"value" => "",
						"type" => "textfield"
